...
例如最近下面脚本最后换行后的结果是乱的：
```sh
sed -n '/^ID/p;/ranks/p' 91-fc-0701.txt  |awk '{print $2}'|sed  '{N;s/\n/  /}'
```
输出前面内容查看：
```sh
sed -n '/^ID/p;/ranks/p' 91-fc-0701.txt  |awk '{print $2}' > test4
```
使用cat查看所有字符：
```
# cat -vET test4
...
有隐藏字符，清理掉：
```sh
sed -n '/^ID/p;/ranks/p' 91-fc-0701.txt  |awk '{print $2}'|tr -d '\r' > test5
```
输出示例：
```
23DB$
...
```
换行测试：
sed -n '/^ID/p;/ranks/p' 91-fc-0701.txt  |awk '{print $2}'|tr -d '\r' |sed  '{N;s/\n/  /}'
```
结果满足需求，示例：
```
23DB  4
