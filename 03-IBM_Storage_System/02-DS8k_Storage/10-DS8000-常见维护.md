...
## DSCLI问题
### DSCLI连接问题
&#8195;&#8195;DS8000存储通过DSCLI无法连接，可以连通，输入密码后连接不上，报错代码是`CMUN00018E`，网络原因，服务未启动，或者是连接数太多了被拒绝了，代码官方描述：[CMUN00018E](https://www.ibm.com/docs/en/ds8870/7.5.0?topic=messages-cmun00018e)。如果排除网络和服务原因，那就是被拒绝了，重启DS8000存储上的ESSNI服务，如果是双HMC，一起重启。

如果是AS400上HA调用DSCLI，可能原因：
- [SJ00203 - POWERHA DS8000 OPERATIONS MAY FAIL WITH HAE2024 WITH 2ND LEV](https://www.ibm.com/mysupport/s/fix-information/aDrKe0000004CgHKAU/fi0129851?language=de)

也可能是死锁连接导致的，在DS8000微码版本88.58.3.0修复，临时解决同样是重启ESSNI服务。

### DSCLI无法登录
#### 老版本时间问题
登录报错示例：
```shell
CHUN00018EUnable to connect to the management console server
```
如果DS8700等老存储，应该是时间问题，把存储时间改到十年前应该就没问题了。
## 待补充