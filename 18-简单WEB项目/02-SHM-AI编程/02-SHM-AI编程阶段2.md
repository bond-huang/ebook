...
## V2.5
### AIX脚本运行问题修复
ch_image_data.sh脚本添加到AIX系统了，运行报错：ksh[47]: bash:  not found.是啥原因

  平台执行脚本时硬编码了 bash，不管脚本本身的 shebang 是什么。AIX 系统没有 bash，所以报错。   
  修复方案：根据脚本的 shebang 自动选择 shell：
...
全部完成！以下是实现的功能：
  ...
  1. 配置 API Key — 打开 AI Assistant 页面，点击右上角齿轮图标 → 点击 "Add Model" → 填入你的 API Key    - 支持预设快速填充：DeepSeek、通义千问、智谱GLM、OpenAI
    - 只需填入 API Key，其他信息自动填充  2. 选择模型 — 顶部下拉框选择要使用的模型
  3. 开始对话 — 点击 "New Chat" 创建新会话，输入消息即可
  ...
  │     功能      │                            说明                            │
  ├───────────────┼────────────────────────────────────────────────────────────┤
  │ 多模型支持    │ DeepSeek、通义千问、智谱GLM、OpenAI 等所有 OpenAI 兼容接口 │
  ├───────────────┼────────────────────────────────────────────────────────────┤
  │ 流式输出      │ 实时打字效果，无需等待完整回复                             │
  ├───────────────┼────────────────────────────────────────────────────────────┤
  │ Markdown 渲染 │ 支持代码块、表格、列表等格式                               │
  ├───────────────┼────────────────────────────────────────────────────────────┤
  │ 对话管理      │ 新建、切换、删除对话                                       │
  ├───────────────┼────────────────────────────────────────────────────────────┤
  │ 历史记录      │ 对话保存到数据库，刷新页面不丢失                           │
  ├───────────────┼────────────────────────────────────────────────────────────┤
  │ 模型配置      │ 页面管理 API Key，预设快速填充                             │
  ├───────────────┼────────────────────────────────────────────────────────────┤
  │ 快捷提问      │ 欢迎页提供预设问题快速开始                                 │
  └───────────────┴────────────────────────────────────────────────────────────┘

  新增文件：
  - server/sql/ai_tables.sql — 数据库表
  - server/services/aiService.js — AI 统一调用服务
  - server/routes/ai.js — AI 相关 API
  - src/api/ai.js — 前端 API 封装

  修改文件：
  - src/views/AIAssistant.vue — 完整聊天界面
  - server/app.js — 注册 AI 路由
