...
添加HA同步项目命令示例：
```
ADDCADMRE CLUSTER(TESTCLU) ADMDMN(CBS) RESOURCE(RSCNAMEA) RSCTYPE(*CLS) RSCLIB(TEMPLIB)
```
如果项目很多，使用iAccess可以批量运行SQL添加，命令示例：
```
cl:ADDCADMRE CLUSTER(TESTCLU) ADMDMN(CBS) RESOURCE(RSCNAMEA) RSCTYPE(*CLS) RSCLIB(TEMPLIB);
cl:ADDCADMRE CLUSTER(TESTCLU) ADMDMN(CBS) RESOURCE(RSCNAMEB) RSCTYPE(*CLS) RSCLIB(TEMPLIB);
cl:ADDCADMRE CLUSTER(TESTCLU) ADMDMN(CBS) RESOURCE(RSCNAMEC) RSCTYPE(*CLS) RSCLIB(TEMPLIB);
cl:ADDCADMRE CLUSTER(TESTCLU) ADMDMN(CBS) RESOURCE(RSCNAMED) RSCTYPE(*CLS) RSCLIB(TEMPLIB);
```
