...
- 50(JBo)：Journal bundle所有者等待DASD日志writes完成。不属于`DASD Write`类型是因为Journal写入方式有特别之处，尽可能高效写入
- 51(JBw)：Journal bundle等待DASD完成。是在执行DASD写入的线程以外的线程中发生的等待 为了提高效率，多个`jobs/threads`可以`ride along`其他`jobs/threads`执行的日志DASD 写入
- 270(EFJ)：EPFS，等待操作系统完成请求的Journal更改
